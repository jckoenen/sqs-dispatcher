...
```kotlin
dependencies {
    implementation("io.github.jckoenen:sqs-dispatcher:VERSION")
}
```
...
The library provides a powerful `consume` DSL that returns a `DrainableFlow`.
...
classes (e.g., `Queue.Url`, `Message.ReceiptHandle`). This prevents accidental mix-ups and ensures that you're always
passing the right kind of identifier to the right function.
